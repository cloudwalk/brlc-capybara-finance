...
describe("Contract 'LendingMarket': base tests", async () => {
  ...
  async function deployLendingMarketAndTakeLoans(): Promise<Fixture> {
    ...
    await proveTx(creditLine.mockLateFeeAmount(LATE_FEE_AMOUNT));
  ...
  async function executeLoanOperations(
    ...
    for (const operation of operations) {
      await increaseBlockTimestampTo(operation.timestamp);
      if (operation.kind === LoanOperationKind.Repayment) {
  ...
  describe("Function 'undoRepaymentFor()'", async () => {
    describe("Execute as expected if", async () => {
      async function executeAndCheck(operationsWithRelativeTimestamps: LoanOperation[]) {
        ...
        const operations: LoanOperation[] = [...operationsWithRelativeTimestamps];
        operations.forEach(operation => {
          if (operation.timestamp >= 0) {
            operation.timestamp += loanStartTimestamp;
          } else {
            operation.timestamp = removeTimestampOffset(
              (loans[0].startPeriod + loans[0].state.durationInPeriods) * PERIOD_IN_SECONDS +
              PERIOD_IN_SECONDS + operation.timestamp
            );
          }
        });
      ...
      describe("There are two repayment before the loan is overdue and", async () => {
        it("The first one is being undone", async () => {
          await executeAndCheck([
            {
              kind: LoanOperationKind.Repayment,
              amount: REPAYMENT_AMOUNT,
              timestamp: 2 * PERIOD_IN_SECONDS,
              subjectToUndo: true
            },
            {
              kind: LoanOperationKind.Repayment,
              amount: REPAYMENT_AMOUNT,
              timestamp: 4 * PERIOD_IN_SECONDS,
              subjectToUndo: false
            }
          ]);
        });
        ...
        it("The first one is being undone and the second is exactly at the loan due date", async () => {
          await executeAndCheck([
            ...
            },
            {
              kind: LoanOperationKind.Repayment,
              amount: REPAYMENT_AMOUNT,
              timestamp: -60, // A negative value are seconds before the end of the loan due date
              subjectToUndo: false
            }
          ]);
        });
