    ...
    ILendingMarketErrorsV2
{
    ...
    // TODO: provide an appropriate view function to get this value externally
    uint256 internal constant INSTALLMENT_COUNT_MAX = uint16(255);

    /// @dev TODO Must not exceed uint240
    // TODO: provide an appropriate view function to get this value externally
    uint256 internal constant START_AUTOGENERATED_SUB_LOAN_ID = uint40(20_000_001);
    ...
    function _takeSubLoan(
        ...
        if (subLoan.status != SubLoanStatus.Nonexistent) {
            revert SubLoanExistentAlready(subLoanId);
        }

        _increaseSubLoanCounter();
        ...
        // Set the sub-loan fields and call the hook function in a separate block to avoid the 'stack too deep' error
        {
            ...
            subLoan.startTimestamp = uint32(startTimestamp); // Safe cast due to prior checks
            ...
            subLoan.trackedTimestamp = uint32(startTimestamp); // Safe cast due to prior checks
    ...
    function _checkLoanParameters(
        ...
        if (loanTakingRequest.firstSubLoanId >= START_AUTOGENERATED_SUB_LOAN_ID) {
            // TODO: Announce that non-autogenerated subloan IDs must be less than START_AUTOGENERATED_SUB_LOAN_ID
            revert FirstSubLoanIdInvalid();
        }
        if (loanTakingRequest.programId == 0) {
            revert ProgramNonexistent();
        }
        if (loanTakingRequest.borrower == address(0)) {
            revert BorrowerAddressZero();
        }
        if (loanTakingRequest.startTimestamp > _blockTimestamp()) {
            // TODO: Announce that loan taking in the future is prohibited, only now or in the past
            revert StartTimestampInvalid();
        }
        if (
            loanTakingRequest.interestRateRemuneratory > type(uint32).max ||
            loanTakingRequest.interestRateMoratory > type(uint32).max ||
            loanTakingRequest.lateFeeRate > type(uint32).max
        ) {
     ...
     * @dev TODO
     */
    function _checkSubLoanParametersAndCalculateTotals(
        SubLoanTakingRequest[] calldata subLoanTakingRequests
    ) internal pure returns (uint256 borrowedAmount, uint256 addonAmount) {
        uint256 len = subLoanTakingRequests.length;
    ...
    function _increaseSubLoanId() internal returns (uint256) {
        LendingMarketStorageV2 storage $ = _getLendingMarketStorage();
        uint256 id = uint256($.lastSubLoanId);
        unchecked {
            id += 1;
        }
     ...
     * @dev TODO
     */
    function _increaseSubLoanCounter() internal {
        LendingMarketStorageV2 storage $ = _getLendingMarketStorage();
        uint256 counter = uint256($.subLoanCounter);
        unchecked {
            counter += 1;
        }
        if (counter > type(uint40).max) {
            revert SubLoanCounterExcess();
        }
        $.subLoanCounter = uint40(counter);
    }
