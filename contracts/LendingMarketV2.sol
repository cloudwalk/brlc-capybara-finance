    ...
    UUPSExtUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        $.storageKind = uint16(STORAGE_KIND_MARKET);
        $.lastSubLoanId = uint40(START_AUTOGENERATED_SUB_LOAN_ID - 1);
    }
    ...
    function takeLoan(
        LoanTakingRequest calldata, SubLoanTakingRequest[] calldata
    ) external whenNotPaused onlyRole(ADMIN_ROLE) returns (uint256 actualFirstSubLoanId) {
    ...
    function lastSubLoanId() external view returns (uint256) {
        return _getLendingMarketStorage().lastSubLoanId;
    }
